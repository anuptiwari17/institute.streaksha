...
import { Search, Plus, MoreHorizontal, AlertTriangle, GraduationCap, UserX, BookOpen } from 'lucide-react';
  ...
  return (
    <Modal open={open} onClose={onClose} title="Add Teacher">
      ...
      <form onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:14}}>
        ...
        <p style={{fontSize:12,color:'#A3A3A0',lineHeight:1.5}}>Login credentials will be sent to the teacher's email automatically.</p>
        ...
        <table style={{width:'100%',borderCollapse:'collapse'}}>
          <thead>
            <tr style={{borderBottom:'1px solid #F0F0EE'}}>
              {['Teacher','Email','Subjects','Status',''].map(h=>(
            ...
            filtered.length===0?(
              ...
              <tr key={t.id} style={{borderBottom:i<filtered.length-1?'1px solid #F5F5F3':'none',transition:'background 0.1s'}}
                ...
                <td style={{padding:'13px 16px',position:'relative'}}>
                  <button onClick={()=>setOpenMenu(openMenu===t.id?null:t.id)} style={{background:'none',border:'none',cursor:'pointer',color:'#A3A3A0',padding:6,borderRadius:8,display:'flex',transition:'all 0.15s'}}
                  ...
                  {openMenu===t.id&&(
                    <div style={{position:'absolute',right:12,top:'100%',zIndex:50,background:'white',border:'1px solid #E5E5E3',borderRadius:12,boxShadow:'0 8px 32px rgba(0,0,0,0.1)',minWidth:180,overflow:'hidden'}}>
                      <button onClick={()=>{setSelectedTeacher(t);setAssignOpen(true);setOpenMenu(null);}}
                      ...
                      {t.is_active&&<button onClick={()=>handleDeactivate(t.id,t.name)}
